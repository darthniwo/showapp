...
(function(angular) {
  ...
  function seriesCtrl(
    ...
    const init = () => {
      ...
      if (!common.maps.tvGenres) {
        common.getTVGenres().then(() => {
          ctrl.state.tvGenres = common.maps.tvGenres;
        });
      } else {
        ctrl.state.tvGenres = common.maps.tvGenres;
      }
     ...
     * @param  {[Integer]} serieId [Serie ID for searching details]
     * @return {[N/A]}         [void]
     */
    ctrl.openTrailer = serieId => {
     ...
     * @param  {[Array]} genres [Array of ids provided on every serie object]
     * @return {[String]}        [String with ganres names sepparated by comma]
     */
    ctrl.renderGenres = genres => {
      const filteredGenres = ctrl.state.tvGenres.src.filter(genre => {
    ...
    /**
     * [checkFav: Method for checking if the selected tv show has been marked as favourite]
     * @param  {[Object]} item [Serie object]
    ...
    /**
     * [getFavClass: Method returning the used to show if the tv show has been marked as favourite]
     * @param  {[Object]} item [Serie object]
    ...
    /**
     * [toggleFav: Method used to mark/unmark a tv show as favourite]
     * @param {[Object]} item [Serie object]
     */
    ctrl.toggleFav = item => {
